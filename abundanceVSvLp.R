...
# foreach(val = c(0.5,1,2),.packages = c("data.table","here","akima")) %dopar% {
#   here()
#   source(here("aesth_par.R"))
#   source(here("loadData.R"))
#   source(here("..","R_files","posPlots.R"))
...
# Use if the sims files are in the projext directory
...
FIAlastQuarData<-do.call(rbind,lapply(
  getFilelist(here(scenario),listPar,val)$p1,file2lastProp,0.75,outPar="Vlp",
  full.path=TRUE))

# use if the sims files are in another directory. Define dir
(simsDir<-paste0("e:/NeuchSims/SARSA/",scenario))
FIAlastQuarData<-do.call(rbind,lapply(
  getFilelist(simsDir,
              listPar,val,fullNam = TRUE)$FIA,file2lastProp,0.7,outPar="Vlp",
  full.path=TRUE))
...
FIA.stats$pA<-round(1-FIA.stats$pR-FIA.stats$pV,2)
...
fwrite(FIAinterpData,here("Simulations",scenario,
                          paste0("Tau",val,"FIAinterpData.txt")),
       row.names = FALSE)

FIAinterpData<-fread(here("Simulations",scenario,
                          paste0("Tau",val,"FIAinterpData.txt")))

colnames(FIAinterpData)<-c(colnames(FIAinterpData)[2:4],"NA")
...
# write.table(FIAinterpData.Neg,
#             here(scenario,paste0("Tau",val,"FIAinterpData.Neg.txt")))

FIAinterpData.Neg<-fread(here("Simulations",scenario,
                          paste0("Tau",val,"FIAinterpData.Neg.txt")))
colnames(FIAinterpData.Neg)<-c(colnames(FIAinterpData.Neg)[2:4],"NA")

# FIAinterpData.Neg.Fut<-AbundLeavData2interp(FIAlastQuarData[Neta==1&Gamma==0.8],
#                                     Var2int = "Prob.RV.V",npoints = pointInt)

# write.table(FIAinterpData.Neg.Fut,
#             here(scenario,paste0("Tau",val,"FIAinterpData.Neg.Fut.txt")))

FIAinterpData.Neg.Fut<-fread(here("Simulations",scenario,
                              paste0("Tau",val,"FIAinterpData.Neg.Fut.txt")))
colnames(FIAinterpData.Neg.Fut)<-c(colnames(FIAinterpData.Neg.Fut)[2:4],"NA")

# Plot real data prob ----------------------------------------------------------

# png(here("Simulations",scenario,paste0("Tau",val,"levelPlotReal.png")),
#     width = 1000 , height = 700)

png(here(scenario,paste0("Tau",val,"levelPlotReal.png")),
    width = 1000 , height = 700)
...
# png(here("Simulations",scenario,paste0("Tau",val,"levelPlotInterp.png")),
#     width = 1000 , height = 700)

png(here(scenario,paste0("Tau",val,"levelPlotInterp.png")),
    width = 1000 , height = 700)


colorbreaksMeans<-seq(0.35,1,length=100)
